package com.sunsun.auto.helper;

import java.io.File;

import com.sunsun.auto.AutoApplication;
import com.sunsun.auto.util.FileUtil;
...
public class StoragePathHelper {
	...
	private static void initInternalStoragePath() {
		if (AutoApplication.getInstance().isSDCardMounted()) {
			return;
		}
	...
	public static void initStoragePath() {
		...
		fileSystemCacheDir = AutoApplication.getAppContext().getCacheDir()
				.getAbsolutePath().concat(File.separator);
		fileSystemDir = AutoApplication.getAppContext().getFilesDir()
				.getAbsolutePath().concat(File.separator);
		...
		if (AutoApplication.getInstance().isSDCardMounted()) {
			...
			// .getAbsolutePath();
			storagePath = AutoApplication.getAppContext()
